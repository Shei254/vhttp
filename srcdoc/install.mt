...
<p>
vhttp requires the following softwares to be installed:
<ul>
<li>C/C++ compiler (GCC or Clang)
<li>CMake
<li>pkg-config
<li>OpenSSL 1.0.2 or later, or a TLS stack compatible with OpenSSL<?= $ctx->{note}->("At the time of writing, vhttp can be built with libressl and boringssl.") ?>
<li>zlib
</ul>
</p>
...
<p>
First, clone the master branch from <a href="https://github.com/vhttp/vhttp/">the source repository</a> as well as the submodules.
</p>
<?= $ctx->{code}->(<< 'EOT')
% git clone --recurse-submodules https://github.com/vhttp/vhttp.git
EOT
?>
...
<p>
When complete, vhttp will be installed under <code>/usr/local</code>.
</p>
...
<?= $ctx->{code}->(<< 'EOT')
% /usr/local/bin/vhttp -c examples/vhttp/vhttp.conf
EOT
?>

<p>
Or if you'd like to start vhttp without installing it, you can use the <code>vhttp_ROOT</code> environment variable.
</p>

<?= $ctx->{code}->(<< 'EOT')
% vhttp_ROOT=$PWD build/vhttp -c examples/vhttp/vhttp.conf
EOT
?>
...
<dd>
This option specifies the directory to which vhttp will be installed (default: <code>/usr/local</code>).
</dd>
...
See also <a href="https://github.com/vhttp/vhttp/wiki/macOS">wiki/macOS</a> to use DTrace on macOS.
</dl>
<dt><code>-DWITH_vhttpLOG=<i>on</i>|<i>off</i></code></dt>
<dd>
This option instructs whether or not to enable <code>vhttplog(1)</code>> support.
It is turned on by default if the prerequisites are found.
See also <a href="./configure/vhttplog.html">vhttplog</a> for details.
</dl>
...
<p>
Thanks to others, vhttp is provided as a binary package on some environments.
Up-to-date versions of vhttp might be found at the following locations.
<ul>
<li><a href="https://www.freshports.org/www/vhttp">FreeBSD vhttp release</a> and <a href="https://www.freshports.org/www/vhttp-devel">vhttp betas</a></li>
<li><a href="https://formulae.brew.sh/formula/vhttp">Homebrew (macOS)</a></li>
<li><a href="https://github.com/tatsushid/vhttp-rpm">RPM (Fedora, RHEL/CentOS, OpenSUSE)</a></li>
<li><a href="https://hub.docker.com/r/lkwg82/vhttp-http2-server/">Docker Image</a></li>
</ul>
</p>
