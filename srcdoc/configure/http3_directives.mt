...
?>
<p>
When a graceful shutdown of vhttp is initiated, vhttp at first sends a GOAWAY frame indicating the clients that it is initiating shutdown, then after one second, starts rejecting new HTTP requests (a.k.a. half-closed state).
</p>
<p>
This directive controls how long vhttp should wait for the peer to close the QUIC connection in this half-closed state, before exitting.
</p>
<p>
If set to zero, this timeout is disabled.
vhttp will not shut down until all QUIC connections are closed by the clients or times out.
</p>
? })
