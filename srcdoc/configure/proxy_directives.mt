...
<p>
Proxy module is the proxy implementation for vhttp - it implements a reverse HTTP proxy and a CONNECT proxy.
</p>
<p>
...
<p>
vhttp will try to reconnect to different backends (in the order determined by the load balancing strategy) until it successfully establishes a connection.
...
$ctx->{directive}->(
    ...
    desc    => "A boolean flag (<code>ON</code> or <code>OFF</code>) indicating if vhttp should add a <code>date</code> header to the response, if that header is missing from the upstream response.",
)->(sub {})
?>
...
$ctx->{directive}->(
    ...
    desc    => "A boolean flag (<code>ON</code> or <code>OFF</code>) indicating if vhttp should send <code>expect: 100-continue</code> header to the request, and postpone sending request body until it receives 100 response",
)->(sub {})
?>
...
?>
<p>
When trying to establish a connection to the CONNECT target, vhttp uses <a href="https://www.rfc-editor.org/rfc/rfc8305" target=_blank>Happy Eyeballs v2 (RFC 8305)</a>.
...
?>
<p>
When the backend protocol is HTTPS, for given ratio of HTTP requests, vhttp will either attempt to create or reuse an existing HTTP/2 connection.
...
?>
<p>
When the backend protocol is HTTPS, for given ratio of HTTP requests, vhttp will either attempt to create or reuse an existing HTTP/3 connection.
</p>
<p>
...
?>
<p>
By default, vhttp buffers unlimited amount of data being sent from backend servers.
The intention behind this approach is to free up backend connections as soon as possible, under the assumption that the backend server might have lower concurrency limits than vhttp.
But if the backend server has enough concurrency, <code>proxy.max-buffer-size</code> can be used to restrict the memory / disk pressure caused by vhttp at the cost of having more connections to the backend server.
</p>
? })
...
<p>
Setting this value to 0 will cause no pipes to be retained by vhttp; the pipes will be closed after they are used.
In this case, vhttp will create new pipes each time they are needed.
</p>
? })
...
?>
<p>
By default, when transmitting a HTTP request to an upstream HTTP server, vhttp removes the received <code>x-forwarded-proto</code> request header and sends its own, as a precaution measure to prevent an attacker connecting through HTTP to lie that they are connected via HTTPS.
However in case vhttp is run behind a trusted HTTPS proxy, such protection might not be desirable, and this configuration directive can be used to modify the behaviour.
</p>
? })
...
?>
<p>
By default, vhttp uses <code>share/vhttp/ca-bundle.crt</code>.  The file contains a set of trusted root certificates maintained by Mozilla, downloaded and converted using <a href="https://curl.haxx.se/docs/mk-ca-bundle.html">mk-ca-bundle.pl</a>.
</p>
? })
...
?>
<p>
If set to <code>ON</code>, the HTTP client implementation of vhttp verifies the peer's certificate using the list of trusted certificates as well as compares the hostname presented in the certificate against the connecting hostname.
</p>
? })
...
Then, if the backend server accepts those requests, vhttp forwards the HTTP response to the client and acts as a bi-directional tunnel.
</p>
<p>
...
<p>
When set to <code>enabled</code>, if zerocopy operation in supported by the downstream connection (i.e., downstream connection being cleoartext or encrypted using kernel TLS), vhttp uses a pipe as an internal buffer instead of using userspace memory.
...
This approach does not reduce the total amount of bytes flowing through the CPU, but reduces the amount of userspace memory used by vhttp by delaying the reads, thereby reducing cache spills.
</p>
? })
