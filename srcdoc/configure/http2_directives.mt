...
<p>
vhttp provides one of the world's most sophisticated <a href="https://tools.ietf.org/html/rfc9113">HTTP/2 protocol</a> implementation, including following features.
</p>

<h3 id="prioritization">Prioritization</h3>

<p>
vhttp is one of the few servers that fully implement prioritization of HTTP responses conformant to what is defined in the <a href="https://tools.ietf.org/html/rfc7540" taget=_blank>HTTP/2 specification</a>.
...
vhttp is capable of detecting such web browsers, and if it does, uses server-driven prioritization; i.e. send responses with certain MIME-types before others.
</p>
<p>
...
<li><a href="http://blog.kazuhooku.com/2015/06/http2-and-vhttp-improves-user-experience.html">HTTP/2 (and vhttp) improves user experience over HTTP/1.1 or SPDY</a>
</ul>
</p>
...
<p>
vhttp recognizes <code>link</code> headers with <a href="https://w3c.github.io/preload/">preload</a> keyword sent by a backend application server (reverse proxy or FastCGI) or an mruby handler, and pushes the designated resource to a client.
</p>
...
<p>When the HTTP/2 driver of vhttp recognizes a <code>link</code> response header with <code>rel=preload</code> attribute set, and if all of the following conditions are met, the specified resource is pushed to the client.
</p>
<ul>
...
<p>
Starting from version 2.1, vhttp provides directives that lets the users tune how the TCP/IP stack is used depending on the observed RTT, CWND, and the additional latency imposed by the interaction between the server and the OS.
</p>
<p>
...
<a href="https://github.com/vhttp/vhttp/issues/421">issue #421</a>
EOT
    desc    => <<'EOT',
...
?>
<p>
When enabled, vhttp maintains a fingerprint of the web browser cache, and cancels server-push suggested by the handlers if the client is known to be in possession of the content.
The fingerprint is stored in a cookie named <code>vhttp_casper</code> using <a href="https://www.imperialviolet.org/2011/04/29/filters.html">Golomb-compressed sets</a> (a compressed encoding of <a href="https://en.wikipedia.org/wiki/Bloom_filter">Bloom filter</a>).
</p>
<p>
...
<p>
This experimental feature serves a JSON document at the fixed path <code>/.well-known/h2/state</code>, which describes an internal HTTP/2 state of the vhttp server.
...
$ctx->{directive}->(
    ...
    see_also => render_mt(<<'EOT'),
<a href="configure/file_directives.html#file.mime.addtypes"><code>file.mime.addtypes</code></a>,
<a href="http://blog.kazuhooku.com/2015/06/http2-and-vhttp-improves-user-experience.html">HTTP/2 (and vhttp) improves user experience over HTTP/1.1 or SPDY</a>
EOT
    desc    => <<'EOT',
...
$ctx->{directive}->(
    ...
    desc    => <<'EOT',
A timeout in seconds. How long to wait before closing the connection on graceful shutdown. Setting the timeout to <code>0</code> deactivates the feature: vhttp will wait for the peer to close the connections.
EOT
)->(sub {});
?>
...
?>
<p>
When the client behavior is suspicious and there is a concern of Denial-of-Service attack, vhttp stops processing requests arriving on the suspicious connection for specified amount of time.
</p>
? })
