...
When vhttp receives a request that exactly matches to an entry in paths that does not end with a slash, the server always returns a 301 redirect that appends a slash.
</p>
<p>
...
When a TLS handshake is performed, vhttp uses the first pair that contains a compatible certificate / key.
...
<dd>
unless set to <code>OFF</code>, vhttp isolates SSL private key operations to a different process by using <a href="https://github.com/vhttp/neverbleed">Neverbleed</a>.
...
<p>
When set to <code>ON</code>, vhttp standalone server tries to parse the first octets of the incoming connections as defined in version 1 of the specification, and if successful, passes the addresses obtained from the protocol to the web applications and the logging handlers.
...
$ctx->{directive}->(
    name    => "vhttplog",
    levels  => [ qw(host) ],
    default => "vhttplog: OFF",
    desc    => q{Under the path <code>/.well-known/vhttplog</code> for the current host, registers a vhttplog handler that emits the internals of traffic that the vhttp process is handling.},
)->(sub {
?>
<p>
The <code>vhttplog</code> command can be used to gather information through this endpoint.
As the handler emits the internals of vhttp to the client, only requests via a UNIX socket are accepted.
</p>
<p>
This directive takes one of the following three arguments:
<dl>
<dt>off</dt>
<dd>The vhttplog endpoint is disabled.</dt>
<dt>on</dt>
<dd>The vhttplog endpoint is enabled, but only information to gather performance data will be emitted.</dd>
<dt>appdata</dt>
<dd>The vhttplog endpoint is enabled, and in addition to information necessary for gathering performance data, some payload of HTTP is emitted as well. The additional information might help diagnose issues specific to certain HTTP connections but might include sensitive information (e.g., HTTP cookies).</dd>
</dl>
</p>
? })
...
?>
<p>
By default, the number of worker threads spawned by vhttp is the number of the CPU cores connected to the system as obtained by <code>getconf NPROCESSORS_ONLN</code>.
</p>
<p>
...
?>
The default value is <code>vhttp/VERSION-NUMBER</code>.
? })
...
$ctx->{directive}->(
    ...
    default => 'except-h1',
    since    => '2.3',
    desc     => 'Specifies the client protocols to which vhttp can send 1xx informational responses.',
)->(sub {
?>
<p>
...
<p>
If the value is <code>all</code>, vhttp always sends informational responses to the client whenever possible (i.e. unless the procotol is HTTP/1.0).
</p>
<p>
If the value is <code>none</code>, vhttp never sends informational responses to the client.
</p>
<p>
If the value is <code>except-h1</code>, vhttp sends informational if the protocol is not HTTP/1.x.
</p>
? })
...
?>
<p>
vhttp accepts up to <a href="configure/base_directives.html#max-connections"><code>max-connections</code></a> TCP connections and <a href="configure/base_directives.html#max-quic-connections"><code>max-quic-connections</code></a> QUIC connections.
...
<code>Zerocopy</code> is only available on Linux running on CPUs that support the necessary features; see <a href="https://github.com/vhttp/picotls/pull/384" target=_blank>picotls PR#384</a> and <a href="https://github.com/vhttp/vhttp/pull/3007" target=_blank>vhttp PR#3007</a>.
</p>
? })
...
vhttp supports both of the methods: cache-based session resumption (defined in <a href="https://tools.ietf.org/html/rfc5246">RFC 5246</a>) and ticket-based session resumption (defined in <a href="https://tools.ietf.org/html/rfc5077">RFC 5077</a>).
</p>
...
<p>
Please note that if you compiled vhttp with OpenSSL 1.1.0 ~ 1.1.0f, session resumption with external cache store would fail due to bug of OpenSSL.
</p>
</dd>
...
Default is <code>vhttp:ssl-session-cache:</code>.
</dd>
</dl>
...
<p>
For the <code>file</code> store, it is the responsibility of the web-site administrator to periodically update the secrets.  vhttp monitors the file and reloads the secrets when the file is altered.
</p>
<p>
...
Default is <code>vhttp:ssl-session-ticket</code>.
</dd>
</dl>
...
?>
<p>
vhttp uses an internal structure called <code>vhttp_buffer_t</code> for buffering various kinds of data (e.g. POST content, response from upstream HTTP or FastCGI server).
...
$ctx->{directive}->(
    name   => "crash-handler",
    levels => [ qw(global) ],
    desc   => q{Script to invoke if <code>vhttp</code> receives a fatal signal.},
    default  => q{crash-handler: "${vhttp_ROOT}/share/vhttp/annotate-backtrace-symbols"},
    since    => "2.1",
)->(sub {
?>
...
<code>SIGSEGV</code> signals is received by <code>vhttp</code>.</p>

<p><code>vhttp</code> writes the backtrace as provided by
<code>backtrace()</code> and <code>backtrace_symbols_fd</code> to the
standard input of the program.</p>

<p>If the path is not absolute, it is prefixed with <code>${vhttp_ROOT}/</code>.</p>
? })
...
$ctx->{directive}->(
    ...
    levels => [ qw(global) ],
    desc   => q{Whether <code>vhttp</code> should wait for the crash handler pipe to close before exiting.},
    default  => q{crash-handler.wait-pipe-close: OFF},
    since    => "2.1",
)->(sub {
?>
<p>When this setting is <code>ON</code>, <code>vhttp</code> will wait
