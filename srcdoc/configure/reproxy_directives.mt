...
?>
<p>
If vhttp recognizes the header, it fetches the contents of the resource specified by the header, and sends the contents as the response to the client.
...
<p>
For example, an upstream server may send an URL pointing to a large image using the <code>X-Reproxy-URL</code> header stored on a distributed file system, and let vhttp fetch and return the content to the client, instead of fetching the image by itself.
