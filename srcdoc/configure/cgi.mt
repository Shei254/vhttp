...
<p>
Starting from version 1.7, vhttp comes with a FastCGI-to-CGI gateway (<code>fastcgi-cgi</code>), which can be found under <code>share/vhttp</code> directory of the installation path.
...
file.custom-handler:
  extension: .cgi
  fastcgi.spawn:
    command: "exec $vhttp_ROOT/share/vhttp/fastcgi-cgi"
EOT
?>
...
<?= $ctx->{example}->('Output of <code>share/vhttp/fastcgi-cgi --help</code>', <<'EOT');
$ share/vhttp/fastcgi-cgi --help
Usage:
    share/vhttp/fastcgi-cgi [options]
