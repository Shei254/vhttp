...
EOT
    ...
    +{
        ...
        - mruby.handler: |
            proc {|env|
              modify_env(env, '$mode')
              status, headers, body = vhttp.$mode.call(env)
...
EOT
    ...
    +{
        ...
        - mruby.handler: |
            proc {|env|
              modify_env(env, '$mode')
              resp1 = vhttp.$mode.call(env)
              content1 = resp1[2].join
              resp2 = vhttp.$mode.call(env)
...
subtest 'file' => sub {
    my $server = create_vhttp("file.dir: @{[ ASSETS_DIR ]}/doc_root");
...
subtest 'proxy' => sub {
    ...
    my $server = create_vhttp("proxy.reverse.url: http://127.0.0.1:$port/");
    for my $mode (qw/next reprocess/) {
        subtest $mode => sub {
            for my $file (keys %files) {
                subtest $file => sub {
...
subtest 'fastcgi' => sub {
    ...
    my $server = create_vhttp("fastcgi.connect: $port");
    for my $mode (qw/next reprocess/) {
        subtest $mode => sub {
            for my $file (keys %files) {
                subtest $file => sub {
    ...
    paths: &paths
      ...
      /non-blocking:
        - mruby.handler: |
            proc {|env|
              st = Time.now
              req1 = vhttp.next.request(env)
              req2 = vhttp.next.request(env)
              req1.join
              req2.join
              et = Time.now
              [200, {}, [et - st]]
...
EOT
    map {
        ...
        /modify?vhttp.remaining_delegations=foo
        /modify?vhttp.remaining_reprocesses=foo
...
subtest 'vhttp.reprocess prefers internal reprocess' => sub {
...
subtest 'both vhttp.next and vhttp.reprocess preserve content-type header' => sub {
    my $server = spawn_vhttp(sub {
        my ($port, $tls_port) = @_;
        << "EOT";
hosts:
