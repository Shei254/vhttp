...
proxy.proxy-status.identity: "vhttp/test"
hosts:
  default:
    paths:
      "/":
...
subtest "basic", sub {
    run_with_curl($server, sub {
        ...
        like $content, qr{proxy-status: vhttp/test; next-hop=127\.0\.0\.1}i;
...
subtest "acl" => sub {
    run_with_curl($server, sub {
        ...
        like $content, qr{proxy-status: vhttp/test; error=destination_ip_prohibited}i;
...
subtest "nxdomain" => sub {
    run_with_curl($server, sub {
        ...
        like $content, qr{proxy-status: vhttp/test; error=dns_error; rcode=NXDOMAIN}i;
...
subtest "refused" => sub {
    run_with_curl($server, sub {
        ...
        like $content, qr{proxy-status: vhttp/test; error=connection_refused; next-hop=127\.0\.0\.1}i;
        like $content, $curl_fail->(502);
    });
};
...
subtest "broken request" => sub {
    ...
    like $resp, qr{^HTTP/1\.1 400 .*\nproxy-status: vhttp/test; error=http_request_error; details="invalid host:port"}s;
};
