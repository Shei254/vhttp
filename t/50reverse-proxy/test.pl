...
my ($aggregated_mode, $vhttp_keepalive, $starlet_keepalive, $starlet_force_chunked, $unix_socket, $zerocopy);
my $ssl_offload = "off";
...
GetOptions(
    ...
    "vhttp-keepalive=i"         => \$vhttp_keepalive,
...
my $huge_file_size = 50 * 1024 * 1024; # should be larger than the mmap_backend threshold of vhttp
  ...
  default:
    paths:
      ...
      /files:
        file.dir: @{[ DOC_ROOT ]}
reproxy: ON
@{[ $vhttp_keepalive ? "" : "proxy.timeout.keepalive: 0" ]}
...
run_with_curl($server, sub {
    ...
    subtest "x-reproxy-url ($proto)" => sub {
        ...
        subtest "keep-alive" => sub {
            ...
            my $remote_port = $body;
            ($headers, $body) = run_prog($cmd);
            if ($vhttp_keepalive && $starlet_keepalive) {
