    ...
    subtest 'command error' => sub {
        my ($tester, $guard) = setup(<<"EOT");
              ...
              proc {|env|
                ...
                rescue vhttp::Redis::CommandError => e
                  [503, {}, [e.message]]
                else
                  [200, {}, []]
                end
    ...
    subtest 'abort connection' => sub {
        my ($tester, $guard) = setup(<<"EOT");
              ...
              proc {|env|
                begin
                  reply = redis.get('k1').join
                rescue vhttp::Redis::ConnectionError => e
                  [503, {}, []]
                else
                  [200, {}, [reply]]
                end
...
EOT
            ...
            subtest 'failure' => sub {
                my ($tester, $guard) = setup(sub {
                    my ($redis_port) = @_;
                    <<"EOT";
              another_redis = vhttp::Redis.new(:host => '127.0.0.1', :port => $redis_port)
              redis.set('k1', 1).join
              proc {|env|
                begin
                    redis.watch('k1') {
                ...
                rescue vhttp::Redis::CommandError
                  [503, {}, []]
                else
                  [200, {}, []]
                end
...
hosts:
  default:
    paths:
      /:
        mruby.handler: |
          redis = vhttp::Redis.new(:host => '127.0.0.1', :port => $redis_port, :db => 1)
          proc {|env|
            db = env['QUERY_STRING']
            unless db.empty?
              redis.db = db.to_i
      ...
      /:
        mruby.handler: |
          proc {|env|
            ...
            redis = vhttp::Redis.new(:host => '$blackhole', :port => 6379, :connect_timeout => env['QUERY_STRING'])
            begin
              redis.get('hoge').join
            rescue vhttp::Redis::ConnectTimeoutError
              [503, {}, []]
            else
              [200, {}, []]
            end
    ...
    paths:
      /:
        mruby.handler: |
          proc {|env|
            # if query string is empty, wait forever
            redis = vhttp::Redis.new(:host => '127.0.0.1', :port => $port, :command_timeout => env['QUERY_STRING'])
            begin
              redis.get('hoge').join
            rescue vhttp::Redis::CommandTimeoutError
              [503, {}, []]
            else
              [200, {}, []]
            end
...
EOT
    for my $path (keys %$confmap) {
      ...
      $path:
        mruby.handler: |
          redis = vhttp::Redis.new(:host => '@{[ $opts->{host} || '127.0.0.1' ]}', :port => $redis_port)
$code
EOT
    }
