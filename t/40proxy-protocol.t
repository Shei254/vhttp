...
sub _spawn_vhttp {
    ...
    open my $fh, ">", "$tempdir/vhttp.conf"
        or die "failed to create file:$tempdir/vhttp.conf:$!";
    print $fh <<"EOT";
handshake-timeout: 3
hosts:
  default:
...
EOT
    ...
    spawn_server(
        argv     => [ bindir() . "/vhttp", "-c", "$tempdir/vhttp.conf" ],
        is_ready => sub {
            check_port($port);
        },
    );
