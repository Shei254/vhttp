...
struct test_timer {
    vhttp_timerwheel_entry_t t;
    int called;
};

static void timer_cb(vhttp_timerwheel_entry_t *t_)
{
    struct test_timer *t = vhttp_STRUCT_FROM_MEMBER(struct test_timer, t, t_);
    t->called++;
    return;
}
...
static enum theft_trial_res prop_wake_time_should_be_before_expiry(struct theft *theft, void *input_)
{
    ...
    uint64_t i;
    size_t events_run;
    vhttp_timerwheel_t *ctx;
    size_t slices = 1;
    uint64_t wake_time;
    struct test_timer t;

    ctx = vhttp_timerwheel_create(6, input->init_time);
    vhttp_timerwheel_run(ctx, input->init_time);
    wake_time = vhttp_timerwheel_get_wake_at(ctx);
    if (wake_time != UINT64_MAX) {
        return THEFT_TRIAL_FAIL;
    }

    vhttp_timerwheel_init_entry(&t.t, timer_cb);
    t.called = 0;

    vhttp_timerwheel_link_abs(ctx, &t.t, input->first_time);
    wake_time = vhttp_timerwheel_get_wake_at(ctx);
    ...
    for (i = input->init_time; i < input->first_time; i += theft_random_choice(theft, slices)) {
        events_run = vhttp_timerwheel_run(ctx, i);
        if (events_run != 0)
            return THEFT_TRIAL_FAIL;
        if (t.called != 0)
            return THEFT_TRIAL_FAIL;
        if (vhttp_timerwheel_get_wake_at(ctx) == UINT64_MAX)
            return THEFT_TRIAL_FAIL;
...
static enum theft_trial_res prop_inserted_timer_should_run_at_expiry(struct theft *theft, void *input_)
{
    ...
    t.called = 0;
    vhttp_timerwheel_link_abs(ctx, &t.t, input->first_time);
    events_run = vhttp_timerwheel_run(ctx, input->second_time);
...
static enum theft_trial_res prop_inserted_timer_should_not_run_before_expiry(struct theft *theft, void *input_)
{
    ...
    vhttp_timerwheel_init_entry(&t.t, timer_cb);
    t.called = 0;
    vhttp_timerwheel_link_abs(ctx, &t.t, input->second_time);
    events_run = vhttp_timerwheel_run(ctx, input->first_time);
