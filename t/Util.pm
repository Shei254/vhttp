...
sub bindir {
    $ENV{vhttp_VALGRIND} || $ENV{BINARY_DIR} || '.';
}
...
sub server_features {
    open my $fh, "-|", bindir() . "/vhttp", "--version"
        or die "failed to invoke: vhttp --version:$!";
    <$fh>; # skip vhttp version
    +{
    ...
    if (prog_exists("memcached")) {
        ...
        if ($pid == 0) {
            ...
            POSIX::dup2($rfh->fileno, 5)
                or die "dup2 failed:$!";
            if ($< == 0) {
                exec qw(share/vhttp/kill-on-close -- memcached -u root -l 127.0.0.1 -p), $port;
            } else {
                exec qw(share/vhttp/kill-on-close -- memcached -l 127.0.0.1 -p), $port;
            }
            exit 1;
        }
        close $rfh;
...
sub exec_fuzzer {
    my $name = shift;
    my $prog = bindir() . "/vhttp-fuzzer-$name";
...
EOT
    ...
    my $ret = spawn_vhttp_raw($conf, \@all_ports, \@opts);
    return {
        %$ret,
        port => $port,
        tls_port => $tls_port,
...
sub spawn_vhttp_raw {
    ...
    # By default, vhttp will launch as many threads as there are CPU cores on the
    ...
    my ($guard, $pid) = spawn_server(
        argv     => [ bindir() . "/vhttp", "-c", $conffn, @{$opts || []} ],
        is_ready => sub {
...
sub spawn_h2_server {
    ...
    my $upstream = IO::Socket::SSL->new(
        ...
        Listen => 1,
        ReuseAddr => 1,
        SSL_cert_file => 'examples/vhttp/server.crt',
        SSL_key_file => 'examples/vhttp/server.key',
...
STDOUT.sync = true
h2g = H2.server({
    'cert_path' => 'examples/vhttp/server.crt',
    'key_path' => 'examples/vhttp/server.key',
});
...
# usage: see t/90vhttplog.t
package vhttplogTracer {
    use POSIX ":sys_wait_h";
    ...
    sub new {
        ...
        my $vhttp_pid = $opts->{pid};
        my $vhttplog_socket_path = $opts->{path};
        my $vhttplog_args = $opts->{args} // [];
        my $output_dir = $opts->{output_dir} // File::Temp::tempdir(CLEANUP => 1);

        my $vhttplog_prog = t::Util::bindir() . "/vhttplog";

        my $output_file = "$output_dir/vhttplog.jsonl";
        my $attaching_opts = $vhttp_pid ? "-p $vhttp_pid" : "-u $vhttplog_socket_path";

        my $tracer_pid = open my($errfh), "-|", qq{exec $vhttplog_prog @{$vhttplog_args} -d $attaching_opts -w '$output_file' 2>&1};
        die "failed to spawn $vhttplog_prog: $!" unless defined $tracer_pid;

        # wait until vhttplog and the trace log becomes ready
        while (1) {
            my $errline = <$errfh>;
            Carp::confess("vhttplog[$tracer_pid] died unexpectedly")
                unless defined $errline;
            Test::More::diag("vhttplog[$tracer_pid]: $errline");
            last if $errline =~ /\bAttaching\b/ms;
        }

        open my $fh, "<", $output_file or die "vhttplog[$tracer_pid] does not create the output file ($output_file): $!";
        my $off = 0;
        my $get_trace = sub {
            Carp::confess "vhttplog[$tracer_pid] is down (got $?)"
        ...
        my $guard = t::Util::make_guard(sub {
            if (waitpid($tracer_pid, WNOHANG) == 0) {
                Test::More::diag "killing vhttplog[$tracer_pid] with SIGTERM";
                kill("TERM", $tracer_pid)
                    or warn("failed to kill vhttplog[$tracer_pid]: $!");
            } else {
                Test::More::diag($_) while <$errfh>; # in case vhttplog shows error messages, e.g. BPF program doesn't compile
                Test::More::diag "vhttplog[$tracer_pid] has already exited";
            }
        });
