...
# setup vhttp that acts as a UDP tunnel
my $tunnel_server = do {
    my $quic_port = empty_port({
        host  => "127.0.0.1",
        proto => "udp",
    });
    my $tunnel_server = spawn_vhttp(<< "EOT");
listen:
  type: quic
  port: $quic_port
  ssl:
...
# setup vhttp that acts as a reverse proxy to the UDP tunnel server
my $proxy_server = do {
    my $quic_port = empty_port({
        host  => "127.0.0.1",
        proto => "udp",
    });
    my $proxy_server = spawn_vhttp(<< "EOT");
listen:
  type: quic
  port: $quic_port
  ssl:
...
# determine UDP port to be used by vhttp-httpclient
...
subtest "udp-rfc9298" => sub {
    ...
    subtest "broken-request" => sub {
        ...
        like $resp, qr{HTTP/1\.1 400 .*\r\nproxy-status: vhttp; error=http_request_error; details="invalid URI"}s;
    };
};
