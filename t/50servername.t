...
EOT
    ...
    like $resp, qr{^server: vhttp/.*\r$}im, "vhttp default Server: header found";
...
hosts:
  default:
    paths:
      /:
        file.dir: @{[ DOC_ROOT ]}
server-name: vhttpalternate
EOT

    my $resp = `curl --silent --dump-header /dev/stderr http://127.0.0.1:$server->{port}/index.txt 2>&1 > /dev/null`;
    like $resp, qr{^HTTP/1\.1 200 }s, "200 response";
    like $resp, qr{^server: vhttpalternate\r$}im, "alternate vhttp Server: header found";
    is +(() = $resp =~ m{^server}img), 1, "header added only once";
};
