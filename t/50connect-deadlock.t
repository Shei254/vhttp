...
sub read_until_blocked {
    ...
    while (1) {
        ...
        if (defined $ret) {
            ...
            if ($! == EAGAIN || $! == EWOULDBLOCK) {
                # Wait for max. 2 seconds for additional data. Due to `vhttp-httpclient` using blocked write to emit what is being
                # received, this timeout should be more than 2x greater than that of `write_until_blocked`. That is because the
                # cause of write stall is the blocked write, which could keep the QUIC stack within `vhttp-httpclient` in an
                # unresponsive state for the timeout specified in `write_until_blocked` (0.5 seconds). That would lead to vhttp
                # sending multiple PTO probes doing an exponential back-off by 2x for 0.5 seconds. Hence 1 second is the minimum
                # here. We use 2 seconds to be on the safe side (TODO: use non-blocking write in `vhttp-httpclient`).
                $! = 0;
