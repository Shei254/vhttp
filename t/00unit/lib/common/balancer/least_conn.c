...
static vhttp_socketpool_target_vector_t gen_targets(size_t size)
{
    size_t i;
    vhttp_socketpool_target_vector_t targets = {NULL};

    vhttp_vector_reserve(NULL, &targets, size);
    for (i = 0; i < size; i++) {
...
static void free_targets(vhttp_socketpool_target_vector_t *targets)
{
    size_t i;
...
{
    vhttp_socketpool_target_vector_t targets = gen_targets(10);
    char tried[10] = {0};
    size_t i;
    size_t selected;
...
static int check_if_acceptable(vhttp_socketpool_target_vector_t *targets, size_t selected)
{
