...
EOT
    ...
    run_with_curl($server, sub {
        ...
        # when the proxy.max-buffer-size is not set, vhttp will buffer the
        ...
        # to write to vhttp, as it will take for curl download  the response
diag $duration;
diag $resp;
        if ($max_on) {
            cmp_ok($duration - $resp, '<=', 2.2, "Writing to vhttp was as fast as the curl download");
        } else {
            cmp_ok($duration - $resp, '>', 3, "Writing to vhttp was much faster than the curl download");
        }
    });
}
