...
EOT
    ...
    is $body, "hello from vhttp_mruby\n";
...
EOT
    my $fetch = sub {
        my $path = shift;
        run_prog("curl --silent -A vhttp_mruby_test --dump-header /dev/stderr http://127.0.0.1:$server->{port}$path");
    };
    ...
    subtest "echo" => sub {
        ...
        like $body, qr{"HTTP_USER_AGENT":"vhttp_mruby_test"}, "HTTP_USER_AGENT";
        like $body, qr{"rack.url_scheme":"http"}, "url_scheme";
        like $body, qr{"SERVER_SOFTWARE":"vhttp/[0-9]+\.[0-9]+\.[0-9]+}, "SERVER_SOFTWARE";
    };
...
EOT
    my ($headers, $body) = run_prog("curl --silent -A vhttp_mruby_test --dump-header /dev/stderr http://127.0.0.1:$server->{port}/");
    like $headers, qr{^HTTP/1\.1 200 OK\r\n}is;
    is md5_hex($body), md5_file("t/50mruby/index.html");
};
...
EOT
            ...
            @log = grep { $_ =~ /^\[vhttp_mruby\]/ } @log;
            like $log[$#log], qr{\[vhttp_mruby\] in request:127\.0\.0\.1:\d+/:mruby raised:\s+(:$lineno:|)\s*hoge \(RuntimeError\)};
        };
    };
...
EOT
    ...
    is $body, 'handler2, /abc, /def/../ghi/../jhk', 'https://github.com/vhttp/vhttp/pull/1480#issuecomment-339614160';

    (undef, $body) = $nc->('/123/../abc/def/../ghi');
    is $body, 'handler2, /abc, /def/../ghi', 'https://github.com/vhttp/vhttp/pull/1480#issuecomment-339658134';
