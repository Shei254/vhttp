...
0. First, this test assumes that a testing node (process) uses the following configurations (see below in spawn_vhttp()).
...
Save the configuration to vhttp-quic-1.conf.

1. Launch vhttp with the above settings (`vhttp -c vhttp-quic-1.conf`), and run `vhttp-httpclient -3 localhost $port` while captureing the packet transfers using tcpdump
```
...
3. Create another vhttp configuration, mostly a copy from the above, except
- node_id: 2
and save it as vhttp-quic-2.conf.

4. Launch vhttp with vhttp-quic-2.conf, and capture the packet trace as in step 1.
...
# launch tracer for vhttp server
my $tracer_pid = fork;
die "fork(2) failed:$!"
...
usdt::vhttp:h3_packet_forward { printf("num_packets=%d num_bytes=%d\n", arg2, arg3); }
EOT
...
# Since it's not associated with any existing connections, vhttp would pass it to `quicly_accept` and
# it would return QUICLY_ERROR_DECRYPTION_FAILED. Then vhttp would try to forward the packet using DCID's node_id field.
...
# The first Initial would successfully create a new connection object inside vhttp.
# Then the because second Initial's DCID doesn't match the first one's, vhttp determines
