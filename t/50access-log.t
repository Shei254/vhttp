...
plan skip_all => 'racy under valgrind' if $ENV{"vhttp_VALGRIND"};
...
sub doit {
    ...
    my $server = spawn_vhttp({conf => <<"EOT", max_ssl_version => $max_ssl_version});
    ...
    doit(
        sub {
            ...
            my $run_clients = sub {
                ...
                subtest "http/3" => sub {
                    is system("$client_prog -3 100 -k https://127.0.0.1:$server->{quic_port}$path > /dev/null"), 0, "run vhttp-httpclient";
