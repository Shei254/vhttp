...
for my $comb (0..3) {
    ...
    subtest $subtest_name, sub {
        ...
        subtest 'not early response' => sub {
            ...
            my $server = spawn_vhttp(vhttp_conf($upstream_port, $up_is_h2));
            if ($down_is_h2) {
        ...
        subtest 'no_drain_body' => sub {
            ...
            if ($down_is_h2) {
                $output = run_with_h2get_simple($server, <<"EOS");
                    ...
                    h2g.read_loop(100)
                    sleep 1
                    # at this time vhttp doesn't know the upstream connection is already closed
...
EOS
            ...
            if ($up_is_h2) {
                ...
                # issue second request to test that vhttp closed the upstream connection
...
subtest 'use-after-free of chunked encoding' => sub {
    ...
    my $server = spawn_vhttp(vhttp_conf($upstream_port, 0));
