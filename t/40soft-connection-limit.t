...
subtest "http1" => sub {
    ...
    subtest 'stats-initial' => sub {
        ...
        # make sure that vhttp sees the @conns being actively closed by the client (rather than evicting them as idle)
        undef @conns;
        sleep 1;
    };
...
sleep(2); # add wait so that vhttp will recognize the clos eof idle connections created above
...
subtest 'http3 soft-connection-limit' => sub {
    my @conns;
    for (1..10) {
        # launch vhttp-httpclient that immediately issues a request and then idles for 10 seconds before sending the next request
        open my $fh, "-|", "@{[bindir()]}/vhttp-httpclient -3 100 -t 2 -d 10000 https://127.0.0.1:$tls_port/ 2> /dev/null"
            or die "failed to launch vhttp-httpclient:$!";
        push @conns, $fh;
    }
