...
MAN = workdir/configure/quick_start.man \
      ...
      workdir/configure/vhttplog.man \

%.html: %.mt snippets/directive.mt snippets/wrapper.mt
	../misc/makedoc.pl $< $@

workdir/%.man: %.mt man-snippets/vhttp.conf.5/directive.mt man-snippets/vhttp.conf.5/header man-snippets/vhttp.conf.5/mruby_method.mt man-snippets/vhttp.conf.5/wrapper.mt
	../misc/makeman.pl $< $@
...
manpage: $(MAN)
	cat $(VPATH)/man-snippets/vhttp.conf.5/header $(MAN) > vhttp.conf.5
	( cat $(VPATH)/man-snippets/vhttp.8/header \
	      $(VPATH)/man-snippets/vhttp.8/description ; \
	  $(BINARY_DIR)/vhttp --help | ../misc/cli2man.pl ; \
	  cat $(VPATH)/man-snippets/vhttp.8/footer ) > vhttp.8
