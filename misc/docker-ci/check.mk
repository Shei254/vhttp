CONTAINER_NAME=vhttpserver/vhttp-ci:ubuntu1604
SRC_DIR=/vhttp
...
ossl3.0:
	docker run $(DOCKER_RUN_OPTS) vhttpserver/vhttp-ci:ubuntu2204 \
		env DTRACE_TESTS=1 \
		make -f $(SRC_DIR)/misc/docker-ci/check.mk _check \
...
asan:
	docker run $(DOCKER_RUN_OPTS) vhttpserver/vhttp-ci:ubuntu2004 \
		make -f $(SRC_DIR)/misc/docker-ci/check.mk _check \
...
coverage:
	docker run $(DOCKER_RUN_OPTS) vhttpserver/vhttp-ci:ubuntu2204  \
		...
		CMAKE_ARGS='-DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_C_FLAGS="-fprofile-instr-generate -fcoverage-mapping -mllvm -runtime-counter-relocation" -DCMAKE_CXX_FLAGS= -DCMAKE_BUILD_TYPE=Debug -DWITH_vhttpLOG=OFF' \
		BUILD_ARGS='$(BUILD_ARGS)' \
		TEST_ENV='SKIP_PROG_EXISTS=1 LLVM_PROFILE_FILE=/home/ci/profraw/%c%p.profraw $(TEST_ENV)' \
		TMP_SIZE='$(TMP_SIZE)'

_coverage_report:
	llvm-profdata merge -sparse -o vhttp.profdata /home/ci/profraw/*.profraw
	llvm-cov report -show-region-summary=0 -instr-profile vhttp.profdata vhttp $(SRC_DIR)/lib $(SRC_DIR)/src $(SRC_DIR)/deps/quicly/lib $(SRC_DIR)/deps/picotls/lib | tee /home/ci/summary.txt
...
_do_check:
	...
	if [ -e vhttp-fuzzer-http1 ] ; then export $(FUZZ_ASAN); fi; \
		ulimit -n 1024; \
